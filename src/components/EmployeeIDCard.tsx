 ...
 */
export default function EmployeeIDCard({ user, language, hospitalSettings }: { user: AppUser, language: 'ar' | 'en', hospitalSettings?: any }) {
  ...
  // Facility Info 
  const facilityNameAr = settings?.institutionNameAr || hospitalSettings?.hospitalNameAr || "مستشفى كايان التخصصي";
  const facilityNameEn = settings?.institutionNameEn || hospitalSettings?.hospitalNameEn || "Kayan Specialist Hospital";
  const website = hospitalSettings?.website || "www.kayanhospital.com";
  // contactEmail not used visually
  const contactPhone = settings?.emergencyPhone || hospitalSettings?.phone || "+966 12 345 6789";
  ...
  return (
    <div className="flex flex-col items-center gap-8 py-6">
      ...
      <div className="w-full overflow-x-auto pb-8">
        <div className="flex flex-wrap md:flex-nowrap justify-center gap-8 md:gap-12 print:block print:p-0 min-w-fit mx-auto px-4">
        ...
        <div id="id-card-front" className="relative w-[324px] h-[512px] bg-white rounded-[20px] shadow-2xl overflow-hidden border border-slate-200 flex flex-col items-center print:shadow-none print:break-after-page mb-8 mx-auto">
          ...
          <div className="relative z-10 w-full px-4 pt-6 flex flex-col items-center text-white">
            ...
            <p className="text-[10px] opacity-80 uppercase font-medium">{isAr ? settings?.taglineAr || "الرعاية التخصصية" : settings?.taglineEn || "Specialist Care"}</p>
          </div>
          ...
          <div className="relative z-10 mt-10 w-32 h-32 border-4 border-white rounded-[24px] shadow-xl overflow-hidden bg-slate-100 flex items-center justify-center">
             {userImageUrl ? (
               <img src={userImageUrl} alt={isAr ? "الصورة الشخصية" : "Profile Picture"} className="w-full h-full object-cover" />
             ) : (
