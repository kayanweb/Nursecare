...
import { Users, Stethoscope, BedDouble, Scissors, Pill, Receipt, Microscope, Settings, TrendingUp, Activity, LayoutGrid, ShieldAlert, LogOut, FileText, Database, CreditCard, ChevronDown, ChevronRight, ActivitySquare, PlusSquare, Network, FlaskConical, CircleDollarSign, CheckSquare, Dna, FileArchive, ArrowRightLeft } from "lucide-react";
...
export default function HospitalInformationSystem({ language, currentUser, systemUsers, hospitalSettings, onLogout }: Props) {
  const [activeModule, setActiveModule] = useState<string>("patients");
  const [activeSubTab, setActiveSubTab] = useState<string>("adt");
  ...
  const systemModules = [
    {
      ...
      items: [
        ...
        { id: "insurance_master", label: isAr ? "شركات التأمين" : "Insurance Rules" },
        { id: "drug_master", label: isAr ? "دليل الأدوية العالمي" : "Drug Master" }
      ]
    },
    {
      id: "patients",
      ...
      icon: Users,
      items: [
        { id: "adt", label: isAr ? "الاستقبال والتسجيل (ADT)" : "Registration (ADT)" },
        { id: "appointments", label: isAr ? "المواعيد" : "Appointments" },
        { id: "queue", label: isAr ? "طابور الانتظار" : "Queue & Routing" },
        { id: "visits", label: isAr ? "إدارة الزيارات (OPD)" : "Visit Management" },
      ]
    },
    {
      id: "clinical",
      ...
      items: [
        ...
        { id: "cpoe", label: isAr ? "الطلبات الإلكترونية (CPOE)" : "CPOE / Orders" },
        { id: "ipd", label: isAr ? "الأقسام الداخلية (IPD)" : "Inpatient Wards" },
        { id: "icu", label: isAr ? "العناية المركزة (ICU)" : "Intensive Care" },
        { id: "er", label: isAr ? "الطوارئ (ER)" : "Emergency" },
        { id: "ot", label: isAr ? "العمليات الجراحية (OT)" : "Operating Theater" },
        { id: "nursing_flow", label: isAr ? "سير التمريض والكارديكس" : "Nursing Flow" },
        { id: "infection_control", label: isAr ? "مكافحة العدوى" : "Infection Control" },
      ]
    },
    {
      id: "diagnostic",
      ...
      icon: Microscope,
      items: [
        { id: "lis_ris", label: isAr ? "المعمل والأشعة (LIS/RIS)" : "Lab & Radiology" },
    ...
    },
    {
      ...
      icon: Pill,
      items: [
        { id: "pharmacy", label: isAr ? "صرف الأدوية (Dispensing)" : "Dispensing" },
        { id: "inventory", label: isAr ? "المخزون والمستودعات" : "Inventory & Stock" },
        { id: "purchasing", label: isAr ? "المشتريات الطبية (PO)" : "Purchasing (PO)" }
      ]
    },
    {
      id: "finance",
      ...
      icon: Receipt,
      items: [
        { id: "billing", label: isAr ? "الحسابات والمطالبات" : "Billing & Claims" },
        { id: "cashier", label: isAr ? "الصندوق والدفع" : "Cashier" },
        { id: "rcm", label: isAr ? "دورة الإيرادات (RCM)" : "Revenue Cycle (RCM)" }
      ]
    },
    {
      id: "dashboards",
      ...
      icon: ActivitySquare,
      items: [
        { id: "cno", label: isAr ? "شاشة الإدارة العليا CNO" : "CNO Dashboard" },
        { id: "supervisor", label: isAr ? "إشراف أرضي وتوجيه" : "Floor Supervisor" },
        { id: "headnurse", label: isAr ? "إدارة القسم" : "Head Nurse Details" },
        { id: "kpi", label: isAr ? "التحليل المالي والطبي" : "Clinical/Financial KPI" },
      ]
    }
  ];
  ...
  const renderMockView = (title: string, structure: string[], description: string) => (
    <div className="p-8 h-full flex flex-col pt-12 items-center justify-center text-center animate-fade-in bg-slate-50/50">
      ...
      <p className="text-slate-500 max-w-lg mx-auto mb-8 font-medium">{description}</p>
      
      <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-lg overflow-hidden max-w-3xl w-full text-left relative" dir="ltr">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-5 relative z-10">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-700 tracking-tight">Active Core Tables</h3>
          </div>
          <span className="text-[10px] font-bold tracking-widest text-emerald-600 uppercase bg-emerald-50 px-2 py-1 rounded">Linked to VisitID Engine</span>
        </div>
        <div className="space-y-4 relative z-10">
            <div className="flex flex-wrap gap-2">
              {structure.map(tb => (
                 <span key={tb} className="bg-slate-50 text-slate-600 font-mono text-[11px] font-bold px-3 py-1.5 rounded-lg border border-slate-200 flex items-center gap-1.5 shadow-sm">
            ...
            <div className="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100/50 p-5 rounded-xl text-left">
               <h4 className="font-black text-blue-900 mb-2.5 flex items-center gap-2 text-sm">
                 <ArrowRightLeft className="w-4 h-4 text-blue-600" /> Order Management Engine Routing
               </h4>
               <p className="text-sm font-medium text-blue-800/80 leading-relaxed">
                 Records written here trigger signals through the standard HL7 workflow:
               </p>
               <div className="mt-3 flex items-center flex-wrap gap-2 text-xs font-mono font-bold text-slate-700">
                 <span className="bg-white border border-blue-100 px-2 py-1 rounded shadow-sm">Visit</span>
                 <span className="text-blue-300">&rarr;</span>
                 <span className="bg-white border border-blue-100 px-2 py-1 rounded shadow-sm">Order</span>
                 <span className="text-blue-300">&rarr;</span>
                 <span className="bg-white border border-blue-100 px-2 py-1 rounded shadow-sm">Service</span>
                 <span className="text-blue-300">&rarr;</span>
                 <span className="bg-white border border-blue-100 px-2 py-1 rounded shadow-sm">Charge</span>
                 <span className="text-blue-300">&rarr;</span>
                 <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-1 rounded shadow-sm">Claim &amp; Invoice</span>
               </div>
            </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col space-y-4 p-2 sm:p-4 bg-slate-50 min-h-screen" dir={isAr ? "rtl" : "ltr"}>
        {/* Header Block */}
          ...
          <div className="flex items-center gap-4 relative z-10 w-full md:w-auto">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200 drop-shadow-sm">
                {isAr ? "نظام الإدارة الصحية المتكامل (HIS-X)" : "Enterprise Hospital Information System (HIS-X)"}
              </h1>
          ...
          <div className="relative z-10 flex border-t md:border-t-0 md:border-x border-white/10 pt-4 md:pt-0 md:px-6 w-full md:w-auto justify-between items-center gap-4">
               {currentUser ? (
                 <div className={`flex flex-col ${isAr ? "text-right" : "text-left"}`}>
                   <span className="font-bold text-sm text-white drop-shadow-md">{isAr ? currentUser.nameAr : currentUser.nameEn}</span>
                   <span className="text-[10px] text-indigo-300 font-mono font-bold tracking-wider mt-0.5">{currentUser.staffId} • {(currentUser.role||"STAFF").toUpperCase()}</span>
                 </div>
               ) : null}
               {onLogout && (
                 <button
                 ...
                 >
                   ...
                   <span className="hidden sm:inline">{isAr ? "إغلاق الواجهة" : "Exit System"}</span>
                 </button>
               )}
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col md:flex-row gap-4 h-[calc(100vh-140px)] min-h-[600px] relative">
          ...
          <div className="w-full md:w-72 bg-white rounded-2xl border border-slate-200/80 shadow-sm flex flex-col overflow-hidden shrink-0">
            <div className="p-4 bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
              <LayoutGrid className="w-4 h-4" /> {isAr ? "مركز التحكم والوحدات" : "Control Center & Modules"}
            </div>
              ...
              {systemModules.map((module) => {
                ...
                return (
                  <div key={module.id} className="mb-2">
                    <button 
                      onClick={() => setActiveModule(isModuleOpen ? "" : module.id)}
                    ...
                    >
                       <div className="flex items-center gap-3">
                         <MIcon className={`w-5 h-5 ${isModuleOpen ? 'text-indigo-600' : 'text-slate-400'}`} />
                         {isAr ? module.labelAr : module.labelEn}
                       </div>
                       <ChevronDown className={`w-4 h-4 transition-transform ${isModuleOpen ? "rotate-180 text-indigo-500" : "text-slate-300"}`} />
                    </button>
                    ...
                    <AnimatePresence>
                      {isModuleOpen && (
                        ...
                        >
                          <div className={`mt-1.5 space-y-1 ${isAr ? "pr-9" : "pl-9"}`}>
                             {module.items.map(item => {
                               const isActiveItem = activeSubTab === item.id;
                               return (
                                 <button
                                   key={item.id}
                                   onClick={() => handleSubTabClick(module.id, item.id)}
                                 ...
                                 >
                                   <div className={`w-1.5 h-1.5 rounded-full ${isActiveItem ? "bg-white" : "bg-slate-300"}`}></div>
                                   {item.label}
                                 </button>
                               )
                             })}
            ...
            <div className="p-4 bg-slate-900 border-t-4 border-indigo-500 text-white text-xs font-mono select-none">
              <div className="flex justify-between items-center mb-2">
                <span className="text-emerald-400 flex items-center gap-1.5 font-bold"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></div> SYSTEM LINKED</span>
                <span className="text-slate-500 bg-slate-800 px-1.5 rounded">v9.2.1</span>
              </div>
              <p className="text-slate-400 leading-relaxed text-[10px]">Cerner/Epic-Style Arch<br/>ICD-10 / CPT ACTIVE</p>
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden relative">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeSubTab === "emr_core" && <EMRDashboard language={language} />}
                ...
                {activeSubTab === "infection_control" && <InfectionControlHub language={language} currentUser={currentUser} systemUsers={systemUsers || []} hospitalSettings={hospitalSettings || {}} />}
                ...
                {/* Master Data */}
                {activeSubTab === "branches" && <BranchesManager language={language} />}
                {activeSubTab === "depts" && <DepartmentsManager language={language} />}
                {activeSubTab === "staff" && <DoctorsStaffRegistry language={language} />}
                {activeSubTab === "insurance_master" && <InsuranceMaster language={language} />}
                {activeSubTab === "drug_master" && <DrugMasterIndex language={language} />}

                {/* Patients */}
                {activeSubTab === "appointments" && <AppointmentsManager language={language} />}
                {activeSubTab === "queue" && <QueueRoutingSystem language={language} />}
                {activeSubTab === "visits" && <VisitManager language={language} />}
                ...
                {activeSubTab === "nursing_flow" && <NursingFlowKardex language={language} />}

                {/* Diagnostic */}
                {activeSubTab === "pathology" && <PathologyDashboard language={language} />}

                {/* Pharmacy */}
                {activeSubTab === "inventory" && <InventoryManager language={language} />}
                {activeSubTab === "purchasing" && <PurchasingPO language={language} />}

                {/* Finance */}
                {activeSubTab === "cashier" && <CashierPointOfSale language={language} />}
                {activeSubTab === "rcm" && <RCMClaims language={language} />}

                {/* Dashboards */}
                {activeSubTab === "kpi" && <AnalyticsKPIDashboard language={language} />}
