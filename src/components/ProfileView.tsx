              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5 text-right">
                    <label className="block text-[11px] font-bold text-slate-500">{isAr ? "الصورة الشخصية (رابط URL أو ملف):" : "Profile Picture (Image URL or File):"}</label>
                    <div className="flex gap-2">
                       <input 
                        ...
                        placeholder={isAr ? "رابط الصورة او رفع ملف" : "URL or Upload file ->"}
                        onChange={(e) => setProfilePictureUrl(e.target.value)} 
                        dir="ltr"
                      />
                      <label className="bg-pink-100 text-pink-600 px-3 flex items-center justify-center rounded-xl cursor-pointer hover:bg-pink-200 transition">
                         <span className="text-xs font-bold whitespace-nowrap">{isAr ? "رفع صوره" : "Upload"}</span>
                         <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                            const file = e.target.files?.[0];
                            if (file) {
                              const reader = new FileReader();
                              reader.onload = (ev) => {
                                setProfilePictureUrl(ev.target?.result as string);
                              };
                  ...
                  {profilePictureUrl && (
                    <div className="flex justify-end sm:justify-start items-center">
                       <img src={profilePictureUrl} className="h-16 w-16 rounded-full border-2 border-pink-500 object-cover shadow-sm bg-slate-100" />
                    </div>
                  )}
                </div>
