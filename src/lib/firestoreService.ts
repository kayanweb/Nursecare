...
const setQuotaExceeded = () => {
    ...
    if (getActiveDbProvider() === "FIREBASE") {
      console.error("🔥 EMERGENCY SWITCH: Firestore quota exhausted, switching to Persistent Local Node Server.");
...
export async function testConnection(): Promise<boolean> {
  ...
  try {
    if (provider === "FIREBASE") {
      ...
      await getDoc(docRef);
      return true; // If we can reach here without network crash
    }
