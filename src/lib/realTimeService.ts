  ...
  const fetchCurrentList = async () => {
    ...
    try {
      ...
      if (json.success && !isUnsubscribed) {
        ...
        // AUTO-HEALING FAILOVER: If fallback server is empty, populate it with the local cached records
        if (!loadedData || loadedData.length === 0) {
           const localData = getLocalStore(collectionName);
           if (localData && localData.length > 0) {
             console.log(`⚕️ [Failover Recovery] Server store is empty for ${collectionName}. Rebuilding state from local cache...`);
             loadedData = localData;
             
             // Seed the empty Express fallback provider with our existing dataset so other clients syncing to it see it
             localData.forEach((item: any) => {
               fetch(`/api/db/${provider.toLowerCase()}/${collectionName}`, {
                 method: "POST",
                 headers: { "Content-Type": "application/json" },
                 body: JSON.stringify(item)
               }).catch(e => console.warn("Failed to seed local data to fallback server instance"));
             });
           }
        }
